...
export default async function AdminPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const selectedCourse = searchParams.curso
    ? courses.find((course) => course.slug === searchParams.curso)
    : undefined;
      ...
      <div className="admin-content">
        ...
        <section className="admin-section" id="cursos">
          <div className="admin-section-title">
            <div>
              <span>{selectedCourse ? 'Cursos' : 'Administrador'}</span>
              <h2>{selectedCourse ? 'Trocar de curso' : 'Meus Cursos'}</h2>
            </div>
          </div>

          <div className={selectedCourse ? 'admin-course-cards compact' : 'admin-course-list'}>
            {courses.map((course) => (
              <Link
              ...
              >
                {course.cover_image_url ? (
                  <img src={course.cover_image_url} alt={`Capa de ${course.title}`} />
                ) : (
            ...
            <form action={saveCourse} className="admin-form admin-course-form" encType="multipart/form-data">
              ...
              <Field label="Descricao do curso" wide>
                <textarea name="description" placeholder="Resumo do que a aluna encontra dentro do curso" />
                  ...
                  <Field label="Capa do curso" help="Padrao recomendado: capa vertical 1080 x 1620 px, igual ao modelo rosa do Vende-C." wide>
