...
export default async function AdminCoursePage({
  ...
  const selectedCourse = courses.find((course) => course.slug === params.curso);
    ...
    <main className="admin-page">
      ...
      <div className="admin-content">
        ...
        <section className="admin-hero">
          ...
          <h1>{selectedCourse ? selectedCourse.title : 'Meus cursos'}</h1>
          <p>
            {selectedCourse
              ? 'Edite este curso individualmente: capa, modulos, aulas, link do video e materiais.'
              : 'Escolha um curso para abrir a pagina individual de edicao. Assim cada curso fica separado e facil de administrar.'}
          </p>
        ...
        <section className="admin-section admin-course-focus" id="cursos">
          <div className="admin-section-title">
            <div>
              <span>Curso aberto</span>
              <h2>{selectedCourse ? selectedCourse.title : 'Curso nao encontrado'}</h2>
            </div>
            <Link className="admin-back-link" href="/admin">Voltar para Meus Cursos</Link>
          </div>

          {selectedCourse ? (
            <div className="admin-open-course">
              {selectedCourse.cover_image_url ? (
                <img src={selectedCourse.cover_image_url} alt={`Capa de ${selectedCourse.title}`} />
              ) : (
              ...
              <div>
                ...
                <span>
                  {selectedCourse.is_published ? 'Publicado' : 'Oculto'} - {courseLessons.length} aulas
                </span>
                <p>Esta pagina edita apenas este curso.</p>
              </div>
            </div>
          ) : (
            <p className="admin-empty">Volte para Meus Cursos e escolha um curso valido.</p>
          )}
        </section>
          ...
          <section className="admin-section">
            ...
            <form action={saveCourse} className="admin-course-panel" encType="multipart/form-data">
              ...
              <div className="admin-panel-block">
                ...
                <p>Use capa vertical no padrao Vende-C: <strong>1080 x 1620 px</strong>. Ela aparece na area de membros e na lista de cursos.</p>
                {selectedCourse.cover_image_url ? (
