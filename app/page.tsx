...
export default async function DebugSupabasePage() {
  ...
  const supabaseRelatedKeys = allEnvKeys.filter((k) => k.toUpperCase().includes('SUPABASE'));
  const totalEnvKeyCount = allEnvKeys.length;
  ...
  return (
    <main style={{ background: '#0b0b0b', color: '#fff', minHeight: '100vh', padding: 32 }}>
      ...
      <p style={{ color: '#ff4b7a', marginTop: 16 }}>debug-version: v11-key-scan</p>
    </main>
  );
}
